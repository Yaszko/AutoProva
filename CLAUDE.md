...
3. Structured output is enforced via forced tool-use, not freeform JSON parsing: [src/lib/examTool.ts](src/lib/examTool.ts) exports `buildSystemPrompt(numQuestoes)` and `buildExamTool(numQuestoes)`, which build the Portuguese system prompt and the `gerar_prova` tool schema (with `minItems`/`maxItems` pinned to `numQuestoes`) for the requested question count, and the request sets `tool_choice: { type: 'tool', name: 'gerar_prova' }`. The response's `tool_use` block's `input` _is_ the `ExamData` object (after `sanitizeExamData()` strips a stray `{,}` decimal-comma artifact the model occasionally emits) — no `JSON.parse`/markdown-fence stripping needed.
