...
async function callAnthropicTool(
  ...
  if (!response.ok) {
    ...
    if (response.status === 401) {
      throw new AnthropicApiError(
        "Chave de API inválida. Verifique sua ANTHROPIC_API_KEY.",
        401,
      );
    }
    ...
    throw new AnthropicApiError(
      detail || `Erro inesperado da API (HTTP ${response.status}).`,
      response.status,
    );
  }
...
export async function generateExam(
  ...
  if (!apiKey.trim()) {
    throw new AnthropicApiError(
      "Informe sua chave de API da Anthropic antes de gerar a prova.",
    );
  }
  if (!userPrompt.trim()) {
...
export async function editQuestion(
  ...
  if (!apiKey.trim()) {
    throw new AnthropicApiError(
      "Informe sua chave de API da Anthropic antes de usar a IA.",
    );
  }
  if (!instruction.trim()) {
    throw new AnthropicApiError(
